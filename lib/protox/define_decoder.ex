defmodule Protox.DefineDecoder do
  ...
  # Generates a function head for each field.
  ...
  defp make_parse_bytes_fun({tag, _, name, {:default, _}, type})
  when type == :bytes or type == :string
  do
    key_bytes = make_key_bytes(tag, type)
  ...
  defp make_parse_bytes_fun({tag, _, name, {:default, _}, type = {:message, m}})
  do
    key_bytes = make_key_bytes(tag, type)
    ...
    quote do
      defp parse(msg, <<unquote(key_bytes), bytes::binary>>) do
        ...
        nested_msg = unquote(m).decode2!(delimited)

        msg
        |> struct([{unquote(name), nested_msg}])
        |> parse(rest)
      end
    end
  end
  ...
  defp make_decode_varint_value_ast(:sint32) do
    quote do: Protox.Zigzag.decode(value)
  end
